 ...
 *
 * STUDENT_NAME_1 (2134160)
 * STUDENT_NAME_2 (2107724)
 * STUDENT_NAME_3 (2157470)
 *
 * Grading:
...
int main (int argc, char * argv[])
{
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: %s <Rsp_queue_name> <S2_queue_name>\n", argv[0]);
        return 1;
    }

    const char *rsp_name = argv[1];
    const char *s2_name = argv[2];

    //open S1 queue for reading
    mqd_t s2_mq = mq_open(s2_name, O_RDONLY);
    if (s2_mq == (mqd_t)-1) {
    ...
    while(1) {
        ...
        ssize_t n = mq_receive(s2_mq, (char *)&job, sizeof(job), NULL);
        if (n < 0) {
        ...
        if((size_t)n != sizeof(MQ_WORKER_MESSAGE)) {
            fprintf(stderr, "worker_s2: wrong message size %zd (expected %zu)\n", n, sizeof(MQ_WORKER_MESSAGE));
            continue;
        }
